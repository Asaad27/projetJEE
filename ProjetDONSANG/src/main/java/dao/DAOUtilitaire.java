...
public class DAOUtilitaire {
    public static PreparedStatement initialisationRequetePreparee(Connection connexion,String sql,
        ...
        PreparedStatement preperedStatement=connexion.prepareStatement(sql,returnGeneratedKeys
                ?Statement.RETURN_GENERATED_KEYS:Statement.NO_GENERATED_KEYS);
        for(int i=0;i<objets.length;i++) {
            preperedStatement.setObject(i+1,objets[i]);
        }
        return preperedStatement;
